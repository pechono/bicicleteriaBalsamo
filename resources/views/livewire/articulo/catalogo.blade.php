        ...
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" wire:click.self="cerrarPromover">
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
                ...
                <div class="p-5 space-y-3">
                    ...
                    @php $costoConIva = round($pCosto * (1 + (float)$pIva/100)); @endphp
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-gray-600 uppercase">IVA %</label>
                            <input type="number" step="0.01" wire:model.live="pIva" class="mt-1 block w-full text-sm rounded border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                            <p class="text-[11px] {{ $pIva > 0 ? 'text-amber-600' : 'text-gray-400' }} mt-1">
                                @if ($pIva > 0) Proveedor discrimina IVA → costo + IVA = ${{ number_format($costoConIva, 0, ',', '.') }} @else IVA ya incluido en el costo @endif
                            </p>
                        </div>
                        <div>
                            ...
                            <p class="text-[11px] text-gray-400 mt-1">Del grupo. Editable.</p>
                        </div>
                        <div>
                        ...
                        <div class="col-span-2 -mt-1">
                            <p class="text-[11px] text-gray-400 leading-tight">
                                Venta = (costo{{ $pIva > 0 ? ' + IVA '.rtrim(rtrim(number_format($pIva,2,',','.'),'0'),',').'%' : '' }}) + {{ rtrim(rtrim(number_format($pPorcentaje,2,',','.'),'0'),',') }}% ganancia.
